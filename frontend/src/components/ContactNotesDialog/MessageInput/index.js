...
const useStyles = makeStyles(theme => ({
	mainWrapper: {
		backgroundColor: theme.palette.bordabox, //DARK MODE//
		display: "flex",
	...
	messageInputWrapper: {
		padding: 6,
		marginRight: 7,
		background: theme.palette.inputdigita || "#fff",
		display: "flex",
		borderRadius: 20,
		flex: 1,
	},
...
const MessageInput = ({ ticketStatus }) => {
	...
	const handleUploadMedia = async e => {
		...
		medias.forEach(media => {
			...
			formData.append("body", ""); // Não enviar nome do arquivo, apenas mídia sem legenda
		});
