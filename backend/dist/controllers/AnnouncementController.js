...
const index = async (req, res) => {
    const { searchParam, pageNumber, includeHidden } = req.query;
    const { companyId } = req.user;
    // Verificar se é superadmin buscando o usuário
    let isSuperAdmin = false;
    try {
        const user = await User_1.default.findByPk(req.user.id, { attributes: ["id", "super"] });
...
const store = async (req, res) => {
    ...
    const { announcement, companiesCount } = await (0, CreateService_1.default)({
        ...data,
        companyId
    });
