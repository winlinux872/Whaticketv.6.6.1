...
const getCacheKey = (prompt, companyId) => {
    return `gemini:suggestions:${companyId}:${prompt.toLowerCase().trim()}`;
};
const GenerateSuggestionsService = async ({ prompt, companyId }) => {
    try {
        // Verificar cache primeiro
        const cacheKey = getCacheKey(prompt, companyId);
        const cachedSuggestions = await cache_1.cacheLayer.get(cacheKey);
        ...
        let apiKey = null;
        try {
            const geminiTokenSetting = await (0, ShowSettingsService_1.default)({
        ...
        if (!apiKey) {
            throw new AppError_1.default("Token do Gemini não configurado. Configure no painel de administração.", 400);
        }
        // Buscar a versão do modelo configurada
        let geminiModelSetting = null;
        try {
            geminiModelSetting = await (0, ShowSettingsService_1.default)({
...
Oi! Como posso tornar seu dia melhor?`;
        ...
        while (retries <= maxRetries) {
            try {
                ...
                throw new AppError_1.default(errorMessage, response.status);
            }
            catch (error) {
        ...
        if (!response || !response.ok) {
            throw new AppError_1.default("Erro ao gerar sugestões após múltiplas tentativas", 500);
        }
        ...
        if (!data.candidates || !data.candidates[0] || !data.candidates[0].content) {
            ...
            throw new AppError_1.default("Resposta inválida da API do Gemini: estrutura de dados não reconhecida", 500);
        }
        if (!data.candidates[0].content.parts || !data.candidates[0].content.parts[0] || !data.candidates[0].content.parts[0].text) {
            ...
            throw new AppError_1.default("Resposta inválida da API do Gemini: texto não encontrado", 500);
        }
        ...
        try {
            await cache_1.cacheLayer.set(cacheKey, JSON.stringify(finalSuggestions), "EX", 300);
        }
        catch (cacheError) {
        ...
        throw new AppError_1.default(`Erro ao gerar sugestões: ${errorMessage}`, 500);
    }
};
exports.default = GenerateSuggestionsService;
