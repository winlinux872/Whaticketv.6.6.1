...
const ListTicketsServiceKanban = async ({ searchParam = "", pageNumber = "1", queueIds, tags, users, status, date, updatedAt, showAll = "true", userId, withUnreadMessages, companyId }) => {
    let whereCondition = {
        queueId: { [sequelize_1.Op.or]: [queueIds, null] },
        companyId,
        status: { [sequelize_1.Op.or]: ["pending", "open", "closed"] }
    };
    let includeCondition = [
        {
            model: Contact_1.default,
            as: "contact",
            attributes: ["id", "name", "number", "lid", "email"]
        },
        {
