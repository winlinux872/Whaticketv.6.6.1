...
const ImportContactsService = async (companyId) => {
    ...
    try {
        ...
        try {
            ...
            logger_1.logger.error(`Failed to create company folder: ${err}`);
            throw err;
        }
        const beforeFilePath = path_1.default.join(companyFolder, 'contatos_antes.txt');
    ...
    const publicFolder = path_1.default.resolve(__dirname, "..", "..", "..", "public");
    const companyFolder = path_1.default.join(publicFolder, `company${companyId}`);
    const afterFilePath = path_1.default.join(companyFolder, 'contatos_depois.txt');
    try {
